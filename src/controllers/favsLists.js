...
exports.create = async(req, res) =>{
    try {
        const { body: {nombre, favoritos}, userId } = req;
        ...
        const favsLists = await FavList.findOneAndUpdate({ _id: favList._id, user: userId}, favList, { new: true});
...
exports.list = async(req, res) =>{
    try {
        ...
        res.status(200).json({ message: `${favsLists.length} elementos encontrados`, favsLists})
    } catch (error) {
...
exports.show = async(req, res) => {
    ...
    try {
        ...
        res.status(400).json({ message: `El elemento con id: ${id} no fue encontrado`});
    }
}
